...
const ListeStagiaires_byAdmins = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [stagiairesResponse, groupesResponse] = await Promise.all([
        axios.get("http://192.168.1.81:5000/Stagiaires"),
        axios.get("http://192.168.1.81:5000/Groupes"),
      ]);
  ...
  const fetchStagiairesByGroup = (id_groupe) => {
    ...
    axios
      .get(`http://192.168.1.81:5000/Stagiaires/findgroupe/${id_groupe}`)
      .then((response) => {
  ...
  const handleDelete = (id_stagiaire) => {
    axios
      .delete(`http://192.168.1.81:5000/Stagiaires/${id_stagiaire}`)
      .then(() => {
...
if (!passwordValid) {
  ...
  return;
    ...
    axios
      .put(`http://192.168.1.81:5000/Stagiaires/${selectedStagiaire}`, {
        cef: updatedCef,
...
export default ListeStagiaires_byAdmins;
