...
const CalendarAdmins = () => {
  ...
  const fetchSeances = async () => {
    try {
      const response = await axios.get(`http://192.168.1.81:5000/seances?date=${date.toISOString()}&time=${time}`);
      ...
      Alert.alert('Error', 'An error occurred while fetching seances.');
    }
  };
